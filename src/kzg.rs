...
impl<E: PairingEngine> KZGCommit<E> {
        ...
        G: AffineCurve<ScalarField = E::Fr> + Sized,
    {
        ...
        let h_timer = start_timer!(|| "compute h");
        let powers: Vec<G::Projective> = powers.iter().map(|x| x.into_projective()).collect();
        let h2 = compute_h_23(c_poly, &powers, p);
        end_timer!(h_timer);
...
pub mod tests {
    ...
    pub fn test_multi_helper<E: PairingEngine>() {
        ...
        let now = Instant::now();
        let q3 = KZGCommit::<E>::multiple_open_23::<E::G1Affine>(&c_poly, &pp.poly_ck.powers_of_g, p);
        println!("Multi advanced 2023 computed in {:?}", now.elapsed());
        assert_eq!(q, q3);
    }
